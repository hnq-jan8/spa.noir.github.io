...
function FlightCardSkeleton() {
  return (
    <div className="bg-white rounded-2xl overflow-hidden card-shadow">
      ...
      <div className="grid grid-cols-2 divide-x divide-gray-200 pt-3">
        {[0, 1].map((i) => (
          <div key={i} className="flex flex-col items-center justify-center">
            <Skeleton className="skeleton-on-card h-3 w-12 mb-2" />
            <Skeleton className="skeleton-on-card h-5 w-16" />
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 divide-x divide-gray-200 py-3 min-h-[74px]">
        {[0, 1].map((i) => (
          <div key={i} className="flex items-center justify-center gap-2">
            <Skeleton className="skeleton-on-card w-4 h-4 flex-shrink-0" />
            <div>
              <Skeleton className="skeleton-on-card h-3 w-12 mb-1.5" />
              <Skeleton className="skeleton-on-card h-4 w-16" />
            </div>
          </div>
        ))}
      </div>
...
/**
 * Mirrors FlightInfoContent: the table (stacked cards below md, an 8-column
  ...
  return (
    <SkeletonScreen className="container-page pt-4 pb-8 md:py-8 md:max-w-6xl md:mx-auto">
      <div className="md:bg-white md:rounded-2xl md:p-6 md:card-shadow">
        ...
        {/* The md+ table: a header row of eight, then body rows. Widths follow
            the real column proportions so nothing jumps on swap. */}
        <div className="hidden md:block -mx-6">
          <div className="px-6">
            <div className="grid grid-cols-[3rem_1fr_1fr_1fr_1.6fr_1fr_1fr_1fr] gap-x-4 lg:gap-x-8 items-center border-b border-gray-200 pb-2">
            ...
            {[0, 1, 2, 3, 4].map((row) => (
              <div
                key={row}
                className="grid grid-cols-[3rem_1fr_1fr_1fr_1.6fr_1fr_1fr_1fr] gap-x-4 lg:gap-x-8 items-center border-b border-gray-100 py-3"
              >
                {[0, 1, 2, 3, 4, 5, 6, 7].map((col) => (
                  <Skeleton
                    key={col}
                    className={`skeleton-on-card h-4 ${col === 4 ? "w-full" : "w-3/4"}`}
                  />
                ))}
              </div>
            ))}
